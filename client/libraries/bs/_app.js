define(function(require) 
{
  ...
  return new Class(
  {
    ...
    ajax: function(obj)
    {
      ...
      this.reqData = {}
      this.reqData = $.extend(obj.data, this.reqData)

      if (this.reqData.isrun)
        urlArgs.forEach((key, val)=> this.reqData[val]=key)

      if (self.item && !this.reqData.isWindow)
        this.reqData.itemId = self.item.id

      this.objs = []
      this.pluginsAfterRender = []

      if (this.getPagePluginManager())
        this.getPagePluginManager().run('beforeSend')

      this.reqData.stream = this.dataStream
      this.dataStream = {}
        ...
        success: function(response) 
        {
          ...
          catch (e) 
          {
            if (response.indexOf('%%__dump__%%') === 0)
            {
              ...
              self.reqData.isWindow ? self.dump(response) : console.log(response)
            }
            else
            {
              self.reqData.isWindow ? self.syntaxServerError(response) : console.log(response)
            }
          ...
          else
          {
            ...
            if (self.reqData.isWindow)
              self.item = resData.item

            if (newDep.length)
              self.require(newDep, ()=>{ self.ajaxSuccess(obj, resData) })
            else
              self.ajaxSuccess(obj, resData)
          }
        }
      })
    },

    ajaxSuccess: function(obj, resData)
    {
      let self = this

      $.each(requirejs.s.contexts._.defined, (name, dep) => App.setDep(name, dep))

      if (this.reqData.isWindow)
      ...
      {
        plugin.onAfterObjsRender()
      }) 

      if (obj.afterSuccess)
        obj.afterSuccess(resData.data)

      this.getPagePluginManager().run('afterSuccess', [resData])
